...
class _ConversationsListScreenState
  ...
  List<WorkerConversation>? _conversations;
    ...
    if (mounted) {
      setState(() {
        ...
        _conversations = conversations
            .where((conv) => !blockedIds.contains(conv.clientId))
            .toList();
        _isLoading = false;
      });
    }
  ...
  Widget build(BuildContext context) {
    ...
    ref.listen(appSettingsProvider.select((s) => s.isDummyMode), (
      previous,
      next,
    ) {
      if (previous != next) {
                        ...
                        children: [
                          ...
                          Consumer(
                            builder: (context, ref, _) {
                              String? avatarId = conversation.clientAvatarId;

                              // If not available, fetch from client's profile
                              if (avatarId == null && conversation.clientId.isNotEmpty) {
                                final asyncAvatarId = ref.watch(
                                  clientAvatarIdProvider(conversation.clientId),
                                );
                              ...
                              return UserAvatar(
                                avatarId: avatarId,
                                displayName: conversation.clientDisplayName,
                                radius: 28,
                              );
                            },
                          ),

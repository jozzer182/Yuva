...
/// Screen to complete worker profile after Google sign-in.
/// Collects required fields: cityOrZone and baseHourlyRate.
...
class _CompleteProfileScreenState extends ConsumerState<CompleteProfileScreen> {
  ...
  Future<void> _saveProfile() async {
    ...
    try {
      ...
      final baseHourlyRate = double.tryParse(_baseHourlyRateController.text.trim()) ?? 0.0;
      ...
      // Create WorkerUser with completed profile
      final workerUser = WorkerUser(
        uid: widget.authUser.id,
      ...
      if (displayName != widget.authUser.name) {
        ref.read(currentUserProvider.notifier).state = widget.authUser.copyWith(
          name: displayName,
        );
      }
                    ...
                    YuvaCard(
                      child: Column(
                        children: [
                          ...
                          // City/Zone field (required for workers)
                          TextFormField(
                            ...
                            validator: (value) {
                              if (value == null || value.trim().isEmpty) {
                                return l10n.completeProfileZoneRequired;
                              }
                              return null;
                            },
                          ),
                          ...
                          // Base hourly rate field (required for workers)
                          TextFormField(
                            controller: _baseHourlyRateController,
                            keyboardType: TextInputType.number,
                            decoration: InputDecoration(
                              labelText: l10n.baseHourlyRate,
                              hintText: l10n.completeProfileRateHint,
                              prefixIcon: const Icon(Icons.attach_money),
                            ),
                            validator: (value) {
                              if (value == null || value.trim().isEmpty) {
                                return l10n.completeProfileRateRequired;
                              }
                              final rate = double.tryParse(value.trim());
                              if (rate == null || rate <= 0) {
                                return l10n.completeProfileRateInvalid;
                              }
                              return null;
                            },
                          ),
